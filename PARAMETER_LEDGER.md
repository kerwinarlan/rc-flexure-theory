...
This ledger tracks all physical parameters for reinforced concrete flexure analysis across the three behavior regions per NSCP 2015 Volume I.
...
| $E_c$ | 24870.06 | MPa | CORRELATED | NSCP 2015 Sec 419.2.2.1 | $E_c = 4700\sqrt{f'_c}$ | Modular ratio $n$, $E_c I_g$, $E_c I_{cr}$ |
...
| $I_g$ | $4.51 \times 10^9$ | mm⁴ | CALCULATED | NSCP 2015 Sec 419.2.3 | $I_g = \frac{1}{12} b h^3$ | Region O->C gross section inertia |
| $I_{cr}$ | $1.804 \times 10^9$ | mm⁴ | CALCULATED | NSCP 2015 Sec 424.5 | $I_{cr} = \frac{1}{3} b x^3 + n A_s (d - x)^2$ | Region C->Y cracked section inertia |
...
| $c$ | 103.81 | mm | CALCULATED | NSCP 2015 Sec 422.2 | $0.85 f'_c b \beta_1 c = A_s f_s$ | Region Y->U inelastic NA depth |
...
| $M_{cr}$ | 52.36 | kN·m | CALCULATED | NSCP 2015 Sec 419.2.3 | $M_{cr} = f_r I_g / y_t$ | Region O->C upper limit |
| $M_y$ | 280.50 | kN·m | CALCULATED | Flexural Mechanics | $M_y = f_y I_{cr} / [n(d-x)]$ | Region C->Y upper limit |
| $M_n$ | 287.21 | kN·m | CALCULATED | NSCP 2015 Sec 422.2 | $M_n = A_s f_s (d - a/2)$ | Region Y->U upper limit |
